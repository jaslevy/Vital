...
def update_quantities(location_id, new_tampon_quantity, new_pad_quantity, new_condom_quantity):
    ...
    assert new_tampon_quantity == "None" or type(new_tampon_quantity) == int, 'non-integer tampon quantity'
    assert new_pad_quantity == "None" or type(
        new_tampon_quantity) == int, 'non-integer pad quantity'
    assert new_condom_quantity == "None" or type(new_condom_quantity) == int, 'non-integer condom quantity'
    try:
        ...
        with psycopg2.connect(database_url) as connection:
            with connection.cursor() as cursor:
                ...
                query += 'SET '
                if new_tampon_quantity != "None": query += f'tampon_count = {new_tampon_quantity}, '
                if new_pad_quantity != "None": query += f'pad_count = {new_pad_quantity}, '
                if new_condom_quantity != "None": query += f'condom_count = {new_condom_quantity}, '
    ...
    try:
        ...
        with psycopg2.connect(database_url) as connection:
            with connection.cursor() as cursor:
                ...
                query += 'FROM filter_locations l '
                query += 'INNER JOIN filter_quantities q '
        ...
        with psycopg2.connect(database_url) as connection:
            ...
            with connection.cursor() as cursor:
                query = 'SELECT building_id, building_name, \
                    ...
                    has_aed, notes FROM buildings WHERE 0=1 '
                if menstrual:
                ...
                if not (menstrual or condom or aed):
                    assert len(results) == 0, "expected no results but got some"
                return results
...
def assert_location(l):
    ...
    assert(isinstance(l[2], str)), "building name for location should be a string"
    assert (isinstance(l[3], str)), "room should be a string"
    assert (isinstance(l[4], str)), "floor should be a string"
    assert (type(l[7])==int or l[7] is None), "tampon_count should be an integer"
    assert (type(l[8])==int or l[8] is None), "pad_count should be an integer"
